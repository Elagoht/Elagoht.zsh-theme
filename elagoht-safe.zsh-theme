...
() {
  ...
  PROMPT="%B┬[${PR_USER}]─[${PR_HOST}]─[%F{blue}%(3~|%1~|%~)%f]\$(virtualenv_prompt_info)\$(ruby_prompt_info)\$(git_prompt_info)%b
%B└[$PR_PROMPT] %b"
RPROMPT="%B\$(git_prompt_status)\${EXECTIME}%b"
  ...
  ZSH_THEME_GIT_PROMPT_UNMERGED="[%F{cyan}Y%f]─"
  ZSH_THEME_GIT_PROMPT_UNTRACKED="[%F{blue}o%f]─"
}
